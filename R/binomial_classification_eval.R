binomial_classification_eval <- function(data,
  ...
  if (!na_in_targets && !na_in_predictions){
    ...
    # Confusion matrices
    overall_conf_mat <- fit_confusion_matrix(predicted_classes = data[["predicted_classes"]],
    ...
    # ROC curves
    overall_roc_curve <- fit_roc_curve(predicted_probabilities = data[[predictions_col]],
                                       targets = data[[targets_col]])
    ...
    if (length(unique_fold_cols) > 1){
      ...
      fold_col_results_nested <- fold_col_results %>%
        tidyr::nest(1:ncol(fold_col_results)) %>%
    ...
    roc_nested <- tibble::tibble(sensitivities = overall_roc_curve$sensitivities,
                                 specificities = overall_roc_curve$specificities) %>%
      tidyr::nest(1:2) %>%
      dplyr::rename(roc = data)

    results <- binomial_classification_results_tibble(overall_roc_curve, roc_nested, overall_conf_mat, predictions_nested)
