...
date: "`r Sys.Date()`"
abstract: |
  In this vignette, we try different numbers of folds in cross-validation and assess the differences.
...
While having a standard setting means one less thing to worry about, let's spend a few minutes discussing this choice. Whether it is reasonable comes down to the context of course, but here are some general thoughts on the topic.
...
out[[1]] %>% 
  ...
  labs(caption = paste0(
    ...
    "\nRMSE (Root Mean Square Error) is for a cross-validated linear model (higher is worse).",
    "\nIn k=3, the third fold differs from the others, impacting the prediction negatively.",
    "\nIn k=10, most folds are similar and the two 'outlier' folds don't impact the average error too much."
    )
  )

```

Note that it took me many tries to find the right random seed that made my point the most clear for this example, so don't expect this picture to be the case always.

If we run this random generation of data 100 times, cross-validate a linear model (`y ~ x`) on each dataset, and then average the RMSE (Root Mean Square Error) and MAE (Mean Absolute Error) from the cross-validations, we get the following results:
...
Whereas the previous section used a regression example (continuous y-variable), we will now perform multiclass classification on the `iris` dataset. This fairly well-known dataset has three species of iris flowers with 50 flowers from each. 
...
Let's quickly count the rows per species, to ensure everything is in order:
...
Above, we see the averaged results from all the fold columns. The last model formula seems to have performed the best as it has the highest `Overall Accuracy`, `Balanced Accuracy`, `F1`, and so on. If what we wanted to do, was to average the results with different settings of `k` (to make our evaluation more robust to that choice), we could stop now. Instead, we will have a look at the results from the different settings of `k`. 

## Inspecting the effect of `k` 

There's a list of nested tibbles (data frame) in the cross-validation output called `Results`. This has the results from each fold column. Let's extract it and format it a bit.

Note: In regression tasks, the `Results` tibbles would have the results from *each fold*, for each fold column, but in classification we gather the predictions from all folds within a fold column, before evaluation.
...
We further add a column for telling us which repetition of the `k` setting, we're in. This is done with the `l_starts` method in `groupdata2::group()`, which automatically starts a new group every time the value of a column changes. So `c(".folds_1", ".folds_1", ".folds_2", ".folds_2")` would give `c(1, 1, 2, 2)`. As we group the data frame by the number of folds, these group indices start over for each setting of `k`. Note though, that this assumes that the `Fold Column` is sorted correctly. We should also remember to remove the grouping in the end, if we don't need it in the following step.
...
With the data all ready, we will plot the effect of `k` on the `Balanced Accuracy` metric. Feel free to plot one of the other metrics as well!
