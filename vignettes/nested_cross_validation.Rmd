---
title: "Nested cross-validation with cvms"
author: 
  - "Ludvig Renbo Olsen"
date: "`r Sys.Date()`"
abstract: |
  In this vignette, we use **nested cross-validation** to tune the hyperparameters 
  of a **custom model function** with `cross_validate_fn()`.
  
  This is the second tutorial for using `cross_validate_fn()`. We recommend skimming ["Cross-validating custom model functions with cvms"](https://cran.r-project.org/web/packages/cvms/vignettes/cross_validating_custom_functions.html) before continuing. 
...
output: 
  ...
  rmarkdown::pdf_document:
    ...
    toc_depth: 4
vignette: >
  %\VignetteIndexEntry{nested_cross_validation}
...
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  fig.path = "man/figures/vignette_cv_nested-",
  dpi = 92,
  fig.retina = 2,
...
optim_model_fn <- function(train_data, formula, hyperparameters){
    folded_train_data <- purrr::exec(groupdata2::fold, !!!fold_args, data=train_data)
    
    cross_validate_fn(train_data, formulas = formula, type = , model_fn = , predict_fn = ,preprocess_fn = ,preprocess_once = ,hyperparameters = ,
                      fold_cols = , cutoff = ,positive = , metrics = , rm_nc = , parallel = ,verbose = )
