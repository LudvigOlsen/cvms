...
ggplot(CV_coeffs, aes(x=term, y=estimate)) +
  geom_point() +
  theme_light()
...
ggplot(CV_coeffs, aes(x=term, y=p.value, color=term)) +
  geom_point() +
  scale_y_continuous(breaks = c(0,0.001,0.01,0.05,0.1,0.5,1.0)) + 
  theme_light()
```

```{r}
dftest <- data %>% filter(.folds_1 != 4)
summary(lm("circumference ~ age", data=dftest))
```
