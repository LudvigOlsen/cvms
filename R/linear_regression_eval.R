...
linear_regression_eval <- function(data,
  ...
  if(isTRUE(models_was_null) ||
    ...
    model_metrics_per_fold <- plyr::ldply(models, function(m){
      fold_eval <- calculate_gaussian_model_metrics(m, REML, metrics = metrics)
    ...
    model_metrics_per_fold <- list(calculate_gaussian_model_metrics(NULL, NULL, metrics = metrics)) %>%
      rep(num_folds) %>%
      dplyr::bind_rows()
    ...
    avg_model_metrics <- calculate_gaussian_model_metrics(NULL, FALSE, metrics = metrics)
