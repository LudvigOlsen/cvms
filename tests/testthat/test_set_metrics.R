...
test_that("metrics are correctly set in set_metrics()",{
  ...
  # multinomial
  expect_equal(set_metrics("multinomial",
  ...
  expect_error(set_metrics("multinomial",
               ...
               "'metrics_list' contained unknown metric names: RMSE.", fixed=TRUE)
  expect_error(set_metrics("multinomial",
                           metrics_list = list("all" = FALSE, "Accuracy" = TRUE,
                                               "Accuracy"=FALSE)),
               "'metrics' cannot contain duplicate names.", fixed=TRUE)
  expect_equal(set_metrics("multinomial",
  ...
  expect_equal(set_metrics("multinomial",
               ...
               c("Balanced Accuracy", "Weighted Accuracy", "F1", "Weighted F1",
  ...
  # binomial
  expect_equal(set_metrics("binomial",
                           metrics_list = list("all" = FALSE, "Accuracy" = TRUE)),
               "Accuracy")
  expect_equal(set_metrics("binomial",
  ...
  expect_error(set_metrics("binomial",
                           metrics_list = list("all" = FALSE, "Weighted F1" = TRUE)),
               "'metrics_list' contained unknown metric names: Weighted F1.", fixed=TRUE)
  expect_error(set_metrics("binomial",
  ...
  expect_equal(set_metrics("binomial",
                           metrics_list = list("all" = TRUE, "Accuracy"=FALSE, "Sensitivity"=FALSE,
                                               "AUC"=TRUE)),
               c("Balanced Accuracy", "F1", "Specificity", "Pos Pred Value",
  ...
  # gaussian
  expect_equal(set_metrics("gaussian",
                           metrics_list = list("all" = FALSE, "RMSE" = TRUE)),
               "RMSE")
  expect_equal(set_metrics("gaussian",
  ...
  expect_equal(set_metrics("gaussian",
                           metrics_list = list("all" = TRUE, "RMSE"=FALSE, "RMSLE"=FALSE,
                                               "MAE" = TRUE)),
               c("MAE", "NRMSE", "RMSEIQR", "RMSESTD", "MALE", "RAE", "RSE",
